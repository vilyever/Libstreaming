...
public class EncoderDebugger {
    ...
    private void debug() {
        ...
        Logger.log("encoders " + Arrays.toString(encoders));
        Logger.log("decoders " + Arrays.toString(decoders));

        // Tries available encoders
        Logger.log("ilength " + encoders.length);
        for (int i = 0; i < encoders.length; i++) {
            Logger.log("i " + i + " , jlength " + encoders[i].formats.length);
            for (int j = 0; j < encoders[i].formats.length; j++) {
                ...
                Logger.log("EncoderName " + mEncoderName + "  ,  EncoderColorFormat " + mEncoderColorFormat);
                ...
                try {
                    ...
                    // Starts the encoder
                    configureEncoder();
                    Logger.log("searchSPSandPPS B");
                    searchSPSandPPS();
                    Logger.log("searchSPSandPPS A");
                    ...
                    Logger.log("releaseEncoder B");
                    releaseEncoder();
                    Logger.log("releaseEncoder A");
                }
...
//        return false;
    ...
    private void releaseEncoder() {
        if (mEncoder != null) {
            try {
                Logger.log("Encoder.stop B");
                mEncoder.stop();
                Logger.log("Encoder.stop A");
            }
            catch (Exception ignore) {
            }
            try {
                Logger.log("Encoder.release B");
                mEncoder.release();
                Logger.log("Encoder.release A");
            }
            catch (Exception ignore) {
            }
        }
